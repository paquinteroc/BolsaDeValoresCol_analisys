...
class Ratio_deviation (bt.Indicator):
    ...
    def next(self):
        ...
        # In other words, choose the stock that is flat while the matching pair started rallying
           ...
           # if (self.data0[0] / self.sma0[0] ) < (self.data1[0] / self.sma1[0] ) : self.lines.lagger = 1
           # else : self.lines.lagger = 0
...
# period_market is the characteristic period of the market, we are using it to decide if there is the stocks
    ...
    def next(self):
        ...
        if self.p.printout:
            pass
            #print('Self  order id:', self.orderid)
...
#            print('Data1 dt:', self.data1.datetime.datetime())
        ...
        if self.Ratio.trigger == 1 and self.getposition(self.data1).size==0 and self.getposition(self.data0).size ==0 :
            ...
            if self.Ratio.lagger ==0 :
               self.p.last_trade =self.buy(data=self.data0)
            elif self.Ratio.lagger ==1:
                self.p.last_trade =self.buy(data=self.data1)
        # if any of stocks  changes momentum close the position
        if  (self.data1 < self.sma1 or self.data0 < self.sma0 ):
            if  self.getposition(self.data0):
                ...
                self.p.last_trade =self.close(self.data0)
            elif self.getposition(self.data1):
               # self.log('CLOSE LONG %s, price = %.2f' % ("PEP", self.data1.close[0]))
               self.p.last_trade = self.close(self.data1)

    def stop(self):
        print('==================================================')
        #print('Starting Value - %.2f' % self.broker.startingcash)
        #print('Ending   Value - %.2f' % self.broker.getvalue())
        # print(self.p.last_trade)
...
#
# class args:
#     def __init__(self, period = 10, stake=1, cash=200, commperc =0 , numfigs =1):
#         self.period =period
#         self.stake = stake
#         self.cash = cash
#         self.commperc = commperc
...
#    print(df.head().to_string())
    ...
    kwargs= dict( ## args to describe the pandas input file
        ...
        #todate=datetime.datetime(2018, 11, 15),
        open=3,
        close=3,
        volume=0,
        openinterest=-1
 ...
 #   Add the commission - only stocks like a for each operation
    ...
    cerebro.addwriter(bt.WriterFile, rounding=4, csv = True, out="CME_CEMAREGOS_MP_ratio_ver6.csv" )
    ...
    #df=pd.DataFrame([output[0].getwritervalues()], columns = output[0].getwriterheaders() )


    # output[0].params.last_trade.Sell if 1 last order was to sell
    # output[0].params.last_trade.Buy if 1 last order was to buy
    if ( output[0].params.last_trade ): # if its not "None" a order was last submitted
        string_email= str(os.path.basename(__file__))+"\n"+str(output[0].dnames)+"\n"+str(output[0].params.last_trade)
        sendemail.message_email(string_email)
...
if __name__ == '__main__':
    runstrat()
    # df = pd.read_csv('trial.csv')  # Read csv file
    # df.head()
    # df.tail()
